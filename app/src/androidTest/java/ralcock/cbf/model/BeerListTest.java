...
public class BeerListTest {
    ...
    public void testFiltering() {
        ...
        expect(fBeers.allBeersList(eq(sortOrder), eq(mild), eq(EMPTY_SET), eq(EMPTY_SET)))
                .andReturn(Arrays.asList(aMild));

        expect(fBeers.allBeersList(eq(sortOrder), eq(best), eq(EMPTY_SET), eq(EMPTY_SET)))
                .andReturn(Arrays.asList(aBest, anotherBest));

        replay(fBeers, fBreweries);

        final BeerList.Config config =
                new BeerList.Config().withSortOrder(sortOrder).withSearchText(mild);
    ...
    public void testSorting() {
        ...
        expect(fBeers.allBeersList(eq(sortOrder1), eq(filterText), eq(EMPTY_SET), eq(EMPTY_SET)))
                .andReturn(Arrays.asList(beer1, beer2, beer3));

        final SortOrder sortOrder2 = SortOrder.BREWERY_NAME_DESC;
        expect(fBeers.allBeersList(eq(sortOrder2), eq(filterText), eq(EMPTY_SET), eq(EMPTY_SET)))
    ...
    public void testFilterByStyle() {
        ...
        expect(fBeers.allBeersList(eq(sortOrder), eq(filterText), eq(stylesToHide), eq(EMPTY_SET)))
